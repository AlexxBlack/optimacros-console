...
import { Car, RequestServerResponse } from "./types";

const server = process.argv[2];
const curl = new Curl();
const terminate = curl.close.bind(curl);

let haltFlag: boolean = false;

function requestServer(query: string, method?: string, payload?: string|number|boolean|null): Promise<RequestServerResponse> {
...
function reply(response:RequestServerResponse):void{
  let color: keyof typeof chalk = 'green';
...
do {
  let command: string = readlineSync.question('Enter the command (type "h" or "help" for list of all suitable commands): ');
  let commandParts: string[] = command.split(" ", 3);
  ...
  switch (commandParts[0]) {
    ...
    case "exit":
      ...
      break;
    case "cars": {
      let result: RequestServerResponse;
      ...
      switch (commandParts[1]) {
        ...
        case "add": {
          ...
          for (let key of Object.keys(car) as (keyof Car)[]){
            let data: string | number = readlineSync.question(`Enter car's ${key}: `);
